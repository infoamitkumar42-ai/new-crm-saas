...
async function reactivatePrincy() {
    ...
    const { data, error } = await supabase
        ...
        .update({
            ...
            is_active: true,
            is_paused: false, // Ensure if this column exists, it's false
            leads_today: 0 // Reset today's count to give her a fresh start tonight
        })
        .eq('id', user.id)
        .select()
        .single();
    ...
    if (error) {
        ...
        console.log(`   - Remaining: ${data.total_leads_promised - data.total_leads_received}`);
