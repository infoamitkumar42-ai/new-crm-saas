...
/**
 * LEADFLOW WEBHOOK v22.0 - WITH WORKING HOURS
 * ============================================
 * ✓ Working Hours: 8 AM - 10 PM (IST)
 * ✓ Night leads saved as Night_Backlog
        ...
        try {
            ...
            for (const entry of payload.entry || []) {
                ...
                for (const change of entry.changes) {
                    ...
                    // 4. GET ALL ACTIVE USERS WITH VALID PLAN (exclude pending plan users)
                    const { data: allUsers, error: usersError } = await supabase
                        .from('users')
                        .select('*')
                        .eq('is_active', true)
                        .neq('plan_name', 'none')  // Only users with active plan
                        .or('is_plan_pending.is.null,is_plan_pending.eq.false');  // Skip pending plan users
                    ...
                    const eligibleUsers = allUsers.filter(user => {
                        ...
                        if (leadState === 'Unknown') return false;
