...
(async () => {
    let out = '| Name | Email | Latest_Renewal | Plan_Type | Quota | Received_Since_Renew | Pending_Corrected |\\n';
    out += '|---|---|---|---|---|---|---|\\n';

    for (const b of boosters) {
        const { data: user } = await supabase.from('users').select('id').eq('email', b.email).single();
        if (!user) {
            out += `| ${b.name} | ${b.email} | ERROR | | | | |\\n`;
            continue;
        }
        ...
        out += `| ${b.name} | ${b.email} | ${renewalFormatted} | ${b.type} | ${b.quota} | ${received} | ${pending} |\\n`;
    }

    fs.writeFileSync('booster_final_report.md', out);
    console.log('✅ Report generated: booster_final_report.md');
})();
