...
// ... (Assuming FULL_DATA is same, using truncated variable for brevity in file but concept is same)
// I will read the FULL_DATA from previous file content logic efficiently.
...
async function runFinalSmart() {
    console.log("🔥 FINAL SMART INSERT (Checking Quota)...");

    // 1. Get Active Users & Calculate Pending Quota
    ...
    console.log(`👥 Available Users with Quota: ${availableUsers.length} / ${users.length}`);

    if (availableUsers.length === 0) {
        return console.log("❌ ALL USERS ARE FULL. Expanding search to users who can take OVER-LIMIT assigned?");
        // If strictly need to assign, we might need a fallback.
        // But for now, let's use available ones.
    }

    // Sort by most need
    availableUsers.sort((a, b) => a.leads_today - b.leads_today);
    ...
    for (const line of lines) {
        ...
        let rawPhone = parts[2].replace('p:', '').trim().replace(/[\s-]/g, '');
        let cleanPhone = rawPhone.replace(/^\+91/, '');
        let finalPhone = cleanPhone.length === 10 ? '+91' + cleanPhone : rawPhone;

        // ** CRITICAL: Pick User with Quota **
        let targetUser = availableUsers[userIdx % availableUsers.length];

        // Ensure user is still valid (locally update counter)
        if (targetUser.leads_today >= targetUser.daily_limit) {
            // Re-filter/Re-sort if this user got full
            // (Simple simple: just find next available)
            const nextUser = availableUsers.find(u => u.leads_today < u.daily_limit);
            if (!nextUser) {
                console.log("⚠️ ALL USERS FILLED. Stopping.");
                break;
            }
            targetUser = nextUser;
        }
        ...
        if (error) {
            if (error.code === '23505') {
                ...
                console.log(`   🔴 ERROR: ${finalPhone} - ${error.message}`);
                failed++;
            }
        } else {
            console.log(`   🟢 Inserted: ${parts[1]} -> ${targetUser.name} (${targetUser.leads_today + 1}/${targetUser.daily_limit})`);

            // Notify
            await supabase.from('notifications').insert({
                user_id: targetUser.id,
                title: 'New Lead Assigned ⚡',
                message: `New Lead: ${parts[1]}`,
                type: 'lead_assigned'
            });

            await supabase.rpc('increment_user_leads', { user_id: targetUser.id });
            ...
            // (We just let the loop find the next one or use simple rotate)
            // Ideally we should resort, but rotation is fine if list is balanced.
            userIdx++;
        }
    }
