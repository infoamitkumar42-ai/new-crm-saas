...
// Custom fetch with retry to bypass Jio network timeouts
const customFetch = async (url, options) => {
    ...
    while (retries > 0) {
        try {
            ...
            retries--;
            console.warn(`Fetch timeout, Retrying... (${retries} left)`);
...
async function run() {
    ...
    const { data: allPayments, error: errP } = await supabase.from('payments').select('*').eq('status', 'captured');
    if (errP) throw errP;
    console.log(`- Fetched ${allPayments.length} successful payments.`);

    console.log('Fetching leads...');
    const { data: allLeads, error: errL } = await supabase.from('leads').select('id, assigned_to, assigned_at, source');
    if (errL) throw errL;
    ...
    console.log(`Total leads still pending to deliver (Active + Needs Review): ${summary.totalPendingToDeliver}`);
