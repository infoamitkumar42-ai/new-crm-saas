...
export const onRequestPost = async (context: any) => {
    ...
    try {
        ...
        if (event === 'payment.captured') {
            ...
            // BUG FIX: += accumulate, not = overwrite
            ...
            const userUpdateRes = await fetch(`${supabaseUrl}/rest/v1/users?id=eq.${userId}`, {
                ...
                body: JSON.stringify({
                    ...
                    is_active: true,
                    daily_limit: config.totalLeads,              // FIX: DB stores total quota, not per-day limit
                    total_leads_promised: newTotalLeadsPromised, // FIX: cumulative += per renewal
