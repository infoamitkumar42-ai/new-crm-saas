...
lines.forEach(line => {
    ...
    if (parts.length >= 9) {
        ...
        if (history !== 'N/A') {
            febRenewals.push(user);
        } else if (status === 'Active') {
            manualActives.push(user);
        }
    }
});

console.log('--- FINAL AUDIT BREAKDOWN ---');
console.log('1. Feb Renewals (Groups):', febRenewals.length);
console.log('   - Total Pending for Feb Renewals:', febRenewals.reduce((s, u) => s + u.pending, 0));
console.log('\n2. Manual Activations (Active + N/A History):', manualActives.length);
console.log('   - Total Pending for Manual Actives:', manualActives.reduce((s, u) => s + u.pending, 0));

console.log('\n--- TARGET BOOSTERS (The 24 group) ---');
const boosters = [
...
const boosterRecord = febRenewals.concat(manualActives).filter(u => boosters.includes(u.email));
console.log('Count of target boosters in list:', boosterRecord.length);
console.log('Total Pending for target boosters:', boosterRecord.reduce((s, u) => s + u.pending, 0));
