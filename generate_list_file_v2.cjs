...
// Using the Key that worked in Task 2177 (proven working)
...
async function generateCleanTable() {
    ...
    users.forEach(u => {
        ...
        if (userPayments.length > 0 || (u.plan_name && u.plan_name !== 'none')) {
            ...
            output += `| ${String(count).padEnd(2)} | ${u.name ? u.name.padEnd(20) : 'Unknown             '} | ${(u.plan_name || 'none').padEnd(12)} | ${leadsStr.padEnd(15)} | ${todayStr.padEnd(13)} | Rs ${String(totalPaid).padEnd(8)} | ${String(payCount).padEnd(9)} | ${lastPay.padEnd(22)} |\n`;
        }
    });
