...
async function recoverLeads() {
    console.log("🚀 STARTING CHIRAG LEAD RECOVERY...");

    // 1. Parse Data
    const rows = rawData.split('\n');
    console.log(`Input rows: ${rows.length}`);

    // 2. Get Active Chirag Users
    const { data: users } = await supabase.from('users')
        .select('id, name, daily_limit, leads_today')
        .eq('team_code', 'GJ01TEAMFIRE')
        .eq('is_active', true)
        .gt('daily_limit', 0)
        .order('leads_today', { ascending: true }); // Prioritize those with fewer leads

    if (!users || users.length === 0) return console.log("No active users in Chirag team!");
    ...
    console.log(`\n✅ RECOVERY COMPLETE!`);
    console.log(`   - New Leads Assigned: ${assignedCount}`);
