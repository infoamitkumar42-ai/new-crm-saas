...
(async () => {
    let overDelivered = 0;
    let netPending = 0; // The theoretical 1816
    let truePending = 0; // The actual 3205

    for (const item of manualList) {
        const { data: user } = await supabase.from('users').select('id, is_active').eq('email', item.email).single();
        if (!user) continue;
        ...
        const used = count || 0;
        const pend = item.quota - used; // Can be negative!

        if (pend < 0) {
            overDelivered += Math.abs(pend);
        } else {
            truePending += pend;
        }
        netPending += pend;
    ...
    console.log('--- 📉 OVER-DELIVERY ANALYSIS ---');
    console.log(`Global Net Pending (Theory): ${netPending}`);
    console.log(`True Pending Needed (Gross): ${truePending}`);
    console.log(`Wasted/Over-delivered Leads: ${overDelivered}`);
    console.log(`Check: ${netPending} + ${overDelivered} should equal ${truePending} roughly?`);
})();
