-- ============================================================================
-- 🚀 CHIRAG TEAM FIX - RPC v35 (IST LOCK + 0-LEAD PRIORITY)
-- ============================================================================
...
BEGIN
    ...
    WITH today_counts AS (
        -- Calculate today's lead count for each user (IST Aware)
        SELECT 
            u.id,
            u.name,
            u.email,
            ...
            COALESCE(
                (SELECT COUNT(*) 
                 ...
                 AND l.created_at >= (CURRENT_DATE AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Kolkata')),
                0
            ) AS leads_today
        FROM users u
    ...
    ORDER BY
        -- 1. ABSOLUTE PRIORITY: People who started with 0 today get first dibs
        (CASE WHEN eu.leads_today = 0 THEN 0 ELSE 1 END) ASC,
        
        -- 2. BATCH PRIORITY: People mid-batch (odd count) next
