...
async function insertMissingLeadsSafe() {
    console.log("🚑 INSERTING MISSING LEADS INTO CHIRAG'S TEAM (Safe Mode)...");
    ...
    // 2. Prepare Data & DB Cache for Duplicates
    const lines = RAW_DATA.split('\n').filter(l => l.trim().length > 0);
    const { data: recentLeads } = await supabase.from('leads')
        .select('phone')
        .gte('created_at', '2026-02-01T00:00:00'); // Check last few days

    const dbPhones = new Set(recentLeads.map(l => l.phone.replace(/^\+91/, '').replace(/[\s-]/g, '')));
    ...
    for (const line of lines) {
        ...
        if (!error) {
            ...
            // Add to local cache (prevent dupes within list)
