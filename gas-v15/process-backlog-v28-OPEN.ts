...
serve(async (req) => {
    ...
    try {
        ...
        console.log('🧹 Backlog Sweeper v28 (OPEN) - CROSS DISTRIBUTION');
        ...
        for (const lead of leads) {
            // A. FILTER (OPEN - NO SOURCE CHECK)
            const eligible = users.filter(u => {
                // Hard Limit Check Only
                if ((u.leads_today || 0) >= (u.daily_limit || 0)) return false;

                // NO SOURCE CHECK HERE - Leads flow freely to everyone
                return true;
            });

            if (eligible.length === 0) continue;

            // B. SORT - FAIR BATCH LOGIC
