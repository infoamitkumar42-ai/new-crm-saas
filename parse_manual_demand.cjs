...
Suman	sumansumankaur09@gmail.com	starter	Active	9/2/2026(â‚¹999) | 12/2/2026(â‚¹999)	â‚¹1998	100	52	48
...
const parsedData = lines.map(line => {
    // Regex to match columns even with tabs or multiple spaces
    const parts = line.split(/[ \\t]+/);

    // Some names have multiple words, so we can't just split by space easily.
    // However, the user provided a tabbed-like format above.
    // Let's try to match by identifying the email column.
    const emailIndex = parts.findIndex(p => p.includes('@'));
    if (emailIndex === -1) return null;

    // Quota and Used are usually the last few columns.
    const lastPart = parts[parts.length - 1]; // Pending
    const secondLast = parts[parts.length - 2]; // Used
    const thirdLast = parts[parts.length - 3]; // Quota

    return {
        email: parts[emailIndex],
        quota: parseInt(thirdLast) || 0,
        used: parseInt(secondLast) || 0,
        pending: parseInt(lastPart) || 0,
        history: parts[emailIndex + 2] // Assuming Status is emailIndex + 1 and History is + 2
    };
}).filter(x => x !== null);

const totalPending = parsedData.reduce((sum, item) => sum + item.pending, 0);
const usersWithPending = parsedData.filter(item => item.pending > 0).length;

console.log('--- 🧬 MANUAL TEXT PARSE RESULTS ---');
console.log('Total Users Found:', parsedData.length);
console.log('Total Pending Leads Sum:', totalPending);
console.log('Total Users with Pending > 0:', usersWithPending);

// Check Himanshu Sharma (sharmahimanshu9797)
const himanshu = parsedData.find(d => d.email === 'sharmahimanshu9797@gmail.com');
if (himanshu) {
    console.log('\\nHimanshu Sharma (sharmahimanshu9797) Row:');
    console.log(' - Quota:', himanshu.quota, 'Used:', himanshu.used, 'Pending:', himanshu.pending);
}

// Check Kulwant Singh
const kulwant = parsedData.find(d => d.email.includes('kulwantsinghdhaliwalsaab668'));
if (kulwant) {
    console.log('\\nKulwant Singh Row:');
    console.log(' - Quota:', kulwant.quota, 'Used:', kulwant.used, 'Pending:', kulwant.pending);
}
