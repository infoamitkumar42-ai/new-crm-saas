...
(async () => {
    ...
    const { data: users, error: uError } = await supabase.from('users').select('id, name, email').in('id', userIds);
    if (uError || !users) { console.error(uError); return; }
    ...
    console.log(`\nResults:`);
    console.log(`- Leads to Verified Manual List members: ${leads.length - ghostLeads}`);
