...
async function verifySmart() {
    console.log("🕵️‍♂️ SMART VERIFY (Checking +91 and without +91)...\n");
    ...
    // Bulk Search using OR Logic
    // Unfortunately, we have to fetch all leads from last 24h/48h and match in memory
    // because WHERE phone IN (...) is tricky with variations.

    const { data: recentLeads } = await supabase.from('leads')
        .select('phone, created_at')
        .gte('created_at', '2026-02-04T00:00:00'); // Last 2 days
    ...
    console.log(`✅ MATCHED IN DB: ${found}`);
    console.log(`❌ STILL MISSING: ${missing}`);

    if (missing > 0) {
        console.log(`\n👉 READY TO INSERT ${missing} LEADS INTO CHIRAG'S TEAM.`);
    }
}
