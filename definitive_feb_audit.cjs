...
(async () => {
    console.log('--- 📊 FEB RENEWAL & MANUAL AUDIT ---');
    ...
    for (const item of manualList) {
        ...
        const pending = Math.max(0, item.quota - received);
        totalDemand += pending;
    ...
    console.log('\\n1. Total Users with Feb Renewal/Manual Activation:', results.length);
    console.log('2. TOTAL PENDING LEADS (The target Dada wants):', totalDemand);

    results.sort((a, b) => b.pending - a.pending);
    console.log('\\nTop Pending Users:');
    console.table(results.slice(0, 15).map(r => ({ Name: r.name, Pending: r.pending, Manual: r.manual })));

    const usersNeedingLeads = results.filter(r => r.pending > 0).length;
    console.log('\\nFinal Count of Users needing activation (Pending > 0):', usersNeedingLeads);
})();
