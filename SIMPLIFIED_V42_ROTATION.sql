-- ============================================================================
-- 🚀 SIMPLIFIED DAILY ROTATION - RPC v42
-- ============================================================================
-- Features:
-- 1. Ignores total_leads_received (History doesn't matter).
-- 2. Focuses ONLY on leads_today.
-- 3. Random distribution for tie-breaking at the start of the day.
-- 4. Atomic Locking (SKIP LOCKED) preserved for parallel safety.
...
BEGIN
    ...
    -- PHASE 1: ATOMIC SELECTION (DAILY FOCUS)
    ...
    ORDER BY 
        -- 1. Fewest leads today (Primary Daily Round Robin)
        (SELECT COUNT(*) FROM leads l 
         WHERE l.assigned_to = u.id 
         AND to_char(l.created_at AT TIME ZONE 'Asia/Kolkata', 'YYYY-MM-DD') = to_char(NOW() AT TIME ZONE 'Asia/Kolkata', 'YYYY-MM-DD')
        ) ASC,
        -- 2. Random Tie-breaker (Everyone starts equal at 0 leads today)
        RANDOM()
    LIMIT 1
