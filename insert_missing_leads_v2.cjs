...
async function insertMissingLeadsV2() {
    console.log("🚑 INSERTING MISSING LEADS INTO CHIRAG'S TEAM (Improved Parsing)...");
    ...
    const { data: users } = await supabase.from('users')
        ...
        .eq('is_active', true)
        .gt('daily_limit', 0)
        .order('leads_today', { ascending: true }); // Round Robin (Least first)

    if (!users || users.length === 0) return console.log("No active users.");

    // 2. Prepare Data & DB Cache (ALL LEADS - NO DATE FILTER)
    // We fetch phone only to be fast
    console.log("Fetching DB cache...");
    // Since leads table is huge, better to check in batches or assume < 50k leads total. 
    // For now, let's fetch phone of ALL leads to be 100% sure.

    // Actually, querying per lead is safer for "missing" logic.
    const allPhonesInInput = [];
    ...
    lines.forEach(l => {
        ...
        if (parts.length >= 3) {
            ...
            allPhonesInInput.push(cleanPhone);
        }
    });

    // We can't fetch WHOLE table. Let's fetch input phones only.
    // Construct OR query is hard.
    // Let's do batch check.

    const dbPhones = new Set();
    const { data: matched } = await supabase.from('leads').select('phone'); // This might be heavy
    // Optimization: Just trust the debug logic and insert if not found.

    // Re-creating the check logic in loop because bulk fetch is heavy.
    if (matched) {
        matched.forEach(l => {
            if (l.phone) dbPhones.add(l.phone.replace(/^\+91/, '').replace(/[\s-]/g, ''));
        });
    }
    ...
    for (const line of lines) {
        ...
        // Ensure +91 format for DB consistency
        ...
        const { error } = await supabase.from('leads').insert({
            ...
            phone: finalPhone,
            city: parts[3],
            source: 'Manual Recovery (Missing Batch)',
            assigned_to: targetUser.id,
            status: 'Assigned',
            created_at: now,
            assigned_at: now // VISIBILITY KEY
        });
        ...
        if (!error) {
            ...
            await supabase.from('notifications').insert({
                ...
                message: `You got a new lead: ${parts[1]}. Check dashboard!`,
                type: 'lead_assigned'
            });
    ...
    console.log(`\n🎉 SUCCESS: Inserted ${inserted} NEW Leads.`);
    console.log(`✅ All assigned to Chirag's team with VISIBILITY ENABLED.`);
}

insertMissingLeadsV2();
