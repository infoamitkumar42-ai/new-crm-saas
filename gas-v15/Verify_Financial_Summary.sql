-- ============================================================================
-- 💰 VERIFY FINANCIAL SUMMARY (Raw Data Analysis)
-- ============================================================================

WITH provided_data (razorpay_payment_id, amount, status) AS (
  VALUES
...
SELECT 
  'Captured (Gross)' as category, SUM(amount) as total_amount, COUNT(*) as count FROM provided_data WHERE status = 'captured'
UNION ALL
SELECT 
  'Refunded' as category, SUM(amount) as total_amount, COUNT(*) as count FROM provided_data WHERE status = 'refunded'
UNION ALL
SELECT 
  'Failed' as category, SUM(amount) as total_amount, COUNT(*) as count FROM provided_data WHERE status = 'failed'
UNION ALL
SELECT
  'Net Calculation (Gross - Refund)' as category, 
  (SUM(CASE WHEN status = 'captured' THEN amount ELSE 0 END) - SUM(CASE WHEN status = 'refunded' THEN amount ELSE 0 END)) as total_amount,
  0 as count
FROM provided_data;
