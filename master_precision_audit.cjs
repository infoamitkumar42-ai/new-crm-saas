...
// Data from user's manual list (extracted from message)
const manualSheet = [
...
(async () => {
    console.log('--- 🧬 MASTER PRECISION AUDIT ---');

    const { data: dbUsers, error } = await supabase
        .from('users')
        .select('name, email, total_leads_promised, total_leads_received, team_code, is_active');

    if (error) return;

    let totalPendingLeads = 0;
    let usersNeedingLeads = 0;
    const finalReport = [];

    // Combine manual list with DB reality
    dbUsers.forEach(u => {
        const manual = manualSheet.find(m => m.email === u.email);

        let quota = manual ? manual.quota : (u.total_leads_promised || 0);
        let used = manual ? manual.used : (u.total_leads_received || 0);

        // If it's a "Done" user in manual sheet (quota=0 or pending=0), we respect that
        const pending = Math.max(0, quota - used);
    ...
    console.log('\nSummary Stats:');
    console.log('1. Total Users tracked in Manual Sheet:', manualSheet.length);
    console.log('2. Total System Demand (Total Remaining Leads):', totalPendingLeads);
    console.log('3. Number of Users with Pending Quota > 0:', usersNeedingLeads);

    // Check for "Extra Leads" recipients
    const extraRecipients = manualSheet.filter(m => m.quota > 0 && m.used > m.quota);
    console.log('\nUsers who already exceeded their quota (Extra Recipients):');
    extraRecipients.forEach(m => console.log(` - ${m.email} (Used: ${m.used}, Quota: ${m.quota})`));

    // Team-wise breakdown of Pending Demand
    const teamDemand = { 'TEAMFIRE': 0, 'TEAMSIMRAN': 0 };
    finalReport.forEach(r => {
        if (teamDemand[r.team] !== undefined) {
            teamDemand[r.team] += r.systemPending;
        }
    });
    console.log('\nRemaining Lead Demand by Team:');
    console.table(teamDemand);
