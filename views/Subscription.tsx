...
export const Subscription: React.FC<SubscriptionProps> = ({ onClose }) => {
  ...
  const handleSubscribe = async (plan: typeof plans.monthly[0]) => {
    ...
    try {
      ...
      // 🛑 DOWNGRADE CHECK (Smart Protection)
      ...
      if (latestProfile && latestProfile.valid_until) {
        // Fallback: Normalize plan name to lowercase to match keys
        const planNameKey = (latestProfile.plan_name || '').toLowerCase().trim();
        const currentWeight = latestProfile.plan_weight || PLAN_WEIGHTS[planNameKey] || 0;
        const newWeight = plan.weight || 0;
        const isValid = new Date(latestProfile.valid_until) > new Date();

        console.log(`Checking: ${planNameKey} (${currentWeight}) vs New (${newWeight})`);

        // BLOCK DOWNGRADE — Only when plan is active AND trying to go lower
        if (isValid && newWeight < currentWeight) {
          alert(`⚠️ Plan Change Not Allowed\n\nYou are on "${latestProfile.plan_name}" (Higher Plan).\nYou cannot switch to "${plan.name}" until your current plan expires.\n\nPlease enjoy your premium benefits! 🚀`);
          setLoading(null);
          return;
        }
      }

      // ✅ PAYMENT FLOW — Only reaches here if not blocked above
      const amount = plan.price * 100; // Production
      // const amount = 100; // Testing - ₹1
    ...
    <div className="fixed inset-0 z-50 bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-900 overflow-hidden">
      ...
      <div className="h-full w-full overflow-y-auto">
        ...
        <div className="fixed inset-0 overflow-hidden pointer-events-none bg-slate-900">
          {/* Removed intense blur-3xl animations that crash mobile GPUs */}
        </div>

        {/* ━━━ Header ━━━ */}
        <div className="sticky top-0 z-20 bg-slate-900 border-b border-white/10 px-4 py-3 flex items-center justify-between">
          <button
            onClick={handleClose}
