...
Arshdeep kaur	arshkaur6395@gmail.com	starter	Active	12/2/2026(â‚¹999) | 12/2/2026(â‚¹999)	â‚¹1998	100	19	81
...
const parsedData = lines.map(line => {
    ...
    return {
        ...
        plan: parts[2],
        status: parts[3],
        paymentHistory: parts[4],
        paidAmount: parts[5],
        quota: parseInt(parts[6]),
        used: parseInt(parts[7]),
        pending: parseInt(parts[8])
    };
}).filter(x => x !== null);

const febRenewals = parsedData.filter(d => d.paymentHistory !== 'N/A');
const manualActivations = parsedData.filter(d => d.paymentHistory === 'N/A' && d.status === 'Active');

const totalFebPending = febRenewals.reduce((sum, item) => sum + item.pending, 0);
const totalManualPending = manualActivations.reduce((sum, item) => sum + item.pending, 0);

console.log('--- 🛡️ RE-AUDIT RESULTS ---');
console.log('1. Total Feb Renewals Found:', febRenewals.length);
console.log('2. Pending leads for Feb Renewals:', totalFebPending);
console.log('3. Total Manual Activations (Active & N/A Payment):', manualActivations.length);
console.log('4. Pending leads for Manual Activations (based on table):', totalManualPending);
console.log('5. GRAND TOTAL DEMAND (Feb + Manual):', totalFebPending + totalManualPending);

// Check Kulwant Singh specifically
const kulwant = parsedData.find(d => d.name.includes('Kulwant Singh'));
if (kulwant) {
    console.log('\\nChecking Kulwant Singh specifically:');
    console.log(' - Email:', kulwant.email);
    console.log(' - Plan:', kulwant.plan);
    console.log(' - History:', kulwant.paymentHistory);
    console.log(' - Quota from Table:', kulwant.quota);
    console.log(' - Pending from Table:', kulwant.pending);
}

// Check Himanshu Sharma (sharmahimanshu9797)
const himanshuRec = parsedData.find(d => d.email === 'sharmahimanshu9797@gmail.com');
if (himanshuRec) {
    console.log('\\nChecking Himanshu Sharma rec specifically:');
    console.log(' - Quota:', himanshuRec.quota);
    console.log(' - Used:', himanshuRec.used);
    console.log(' - Pending:', himanshuRec.pending);
}
