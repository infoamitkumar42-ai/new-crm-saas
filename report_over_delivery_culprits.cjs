...
(async () => {
    console.log('--- 📉 OVER-DELIVERY CULPRITS (Accounting for ~1389 Leads) 📉 ---');
    console.log('These users received MORE than their quota allowed since their renewal date.');

    let totalOver = 0;
    const report = [];

    for (const item of manualList) {
        const { data: user } = await supabase.from('users').select('id').eq('email', item.email).single();
        if (!user) continue;

        const { count } = await supabase.from('leads').select('*', { count: 'exact', head: true })
            .eq('assigned_to', user.id).gte('created_at', item.date + 'T00:00:00Z');

        const used = count || 0;
        const extra = used - item.quota;

        if (extra > 0) {
            report.push({ name: item.name, email: item.email, quota: item.quota, used: used, extra: extra });
            totalOver += extra;
        }
    }

    report.sort((a, b) => b.extra - a.extra);
    console.table(report.map(r => ({ Name: r.name, "Plan Quota": r.quota, "Used Since Renew": r.used, "EXTRA": r.extra })));

    console.log(`\nTotal Extra Leads Identifyable in Manual List: ${totalOver}`);
    console.log('(Note: Remainder of 1389 likely from Inactive/Churned users not on this list)');
})();
