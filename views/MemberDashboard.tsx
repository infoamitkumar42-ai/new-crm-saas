/**
 * ╔════════════════════════════════════════════════════════════╗
 * ║  🔒 LOCKED - MemberDashboard.tsx v3.2                      ║
 * ║  Updated: January 8, 2026                                  ║
 * ║  Edition: NIGHT OWL & SMART UI                             ║
 * ║                                                            ║
 * ║  Features:                                                 ║
 * ║  - ✅ Night Lead Smart Detection (🌙 Icon)                 ║
 * ║  - ✅ Exact Time Display (02:30 AM) vs Relative Time       ║
 * ║  - ✅ "Mood Protection" Blue Tip UI                        ║
 * ║  - ✅ Mobile Optimized (PWA Ready)                         ║
 * ║  - ✅ Previous Features (Pause, Report, Filters)           ║
 * ║                                                            ║
 * ║  ⚠️  STATUS: STABLE - READY FOR DEPLOYMENT                 ║
 * ╚════════════════════════════════════════════════════════════╝
 */
...
import {
  ...
  CheckCircle2, AlertTriangle, Flag, Gift, Lightbulb
} from 'lucide-react';
...
// 🔥 NEW: Exact Time Formatter (02:30 AM)
...
// 🔥 NEW: Check if lead is from today
...
export const MemberDashboard = () => {
  ...
  const deliveryStatus: DeliveryStatusInfo = useMemo(() => {
    if (!profile) {
      return {
        title: 'Loading...', subtitle: 'Fetching status', icon: Clock,
        iconBgColor: 'bg-white/20', iconColor: 'text-white', statusType: 'inactive'
      };
    }
    if (profile.payment_status !== 'active' || isExpired) {
      return {
        title: 'Plan Inactive', subtitle: 'Renew to receive leads', icon: AlertTriangle,
        iconBgColor: 'bg-red-500/30', iconColor: 'text-red-200', statusType: 'expired'
      };
    }
    if (isPaused) {
      return {
        title: 'Delivery Paused', subtitle: 'Resume to receive leads', icon: Pause,
        iconBgColor: 'bg-orange-500/30', iconColor: 'text-orange-200', statusType: 'paused'
      };
    }
    if (!isWithinWorkingHours()) {
      return {
        title: 'Off Hours', subtitle: 'Delivery: 8 AM - 10 PM', icon: Moon,
        iconBgColor: 'bg-white/20', iconColor: 'text-indigo-200', statusType: 'off_hours'
      };
    }
    if (isLimitReached) {
      return {
        title: 'Daily Limit Reached', subtitle: `Received ${dailyLimit} leads today`, icon: CheckCircle2,
        iconBgColor: 'bg-green-500/30', iconColor: 'text-green-200', statusType: 'limit_reached'
      };
    }
    return {
      title: 'Actively Receiving', subtitle: `${remainingToday} more leads today`, icon: Zap,
      iconBgColor: 'bg-green-500/30', iconColor: 'text-green-300', statusType: 'active'
    };
      ...
      if (userData) {
        ...
        if (userData.manager_id) {
          const { data: managerData } = await supabase
            .from('users').select('name').eq('id', userData.manager_id).maybeSingle();
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', {
        event: 'INSERT', schema: 'public', table: 'leads', filter: `user_id=eq.${profile.id}`,
      }, (payload) => {
  ...
  const toggleDeliveryPause = async () => {
    ...
    try {
      await supabase.from('users').update({
        is_active: newActiveStatus, updated_at: new Date().toISOString()
      }).eq('id', profile.id);
      await fetchData();
    } catch (err: any) {
  ...
  const saveNote = async () => {
    ...
    try {
      ...
      setShowNotesModal(null); setNoteText('');
    } catch (err: any) {
      alert('Error: ' + err.message);
    } finally {
      setSavingNote(false);
  ...
  const handleReportInvalidLead = async () => {
    ...
    try {
      await supabase.from('lead_replacements').insert({
        user_id: profile.id, original_lead_id: showReportModal.id, original_lead_phone: showReportModal.phone,
        reason: reportReason.trim(), reason_details: `Lead Name: ${showReportModal.name}`, status: 'pending'
      });
      ...
      setLeads(prev => prev.map(l => l.id === showReportModal.id ? { ...l, status: 'Invalid', notes: (l.notes || '') + `\n[REPORTED: ${reportReason.trim()}]` } : l));
      setShowReportModal(null); setReportReason('');
  ...
  if (!profile) return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50 text-center p-8">
      <div><AlertTriangle size={48} className="text-amber-500 mx-auto mb-4" /><h2 className="text-xl font-bold">Profile Not Found</h2><button onClick={() => supabase.auth.signOut()} className="mt-4 bg-blue-600 text-white px-6 py-2 rounded-lg">Sign Out & Retry</button></div>
    </div>
  );
  ...
  return (
    <div className={`min-h-screen bg-slate-50 font-sans ${isExpired ? 'overflow-hidden' : ''}`}>
      ...
      <div className="relative z-30">
        {isPaused && !isExpired && (
          <div className="bg-orange-500 text-white py-2.5 px-4"><div className="max-w-7xl mx-auto flex items-center justify-between gap-2"><div className="flex items-center gap-2 text-sm font-medium"><Pause size={16} className="animate-pulse" /><span>⏸️ Lead delivery is paused.</span></div><button onClick={toggleDeliveryPause} className="bg-white text-orange-600 px-3 py-1 rounded-lg font-bold text-xs flex items-center gap-1"><Play size={12} /> Resume</button></div></div>
        )}
        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-6 py-3 flex justify-between items-center gap-2">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <h1 className="text-base sm:text-xl font-bold text-slate-900 truncate">👋 {profile.name?.split(' ')[0] || 'Member'}</h1>
              <span className={`hidden sm:inline-flex px-2 py-0.5 rounded text-[10px] font-bold ${priorityBadge.color}`}>{priorityBadge.text}</span>
            </div>
            <div className="text-xs text-slate-500 truncate"><span className="text-green-600 font-medium capitalize">{profile.plan_name || 'No Plan'}</span><span className="mx-1 hidden sm:inline">•</span><span className="hidden sm:inline">{managerName}</span></div>
          </div>
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            <button onClick={fetchData} disabled={refreshing} className="w-9 h-9 flex items-center justify-center bg-slate-100 hover:bg-slate-200 rounded-lg"><RefreshCw size={18} className={`text-slate-600 ${refreshing ? 'animate-spin' : ''}`} /></button>
            <button onClick={() => supabase.auth.signOut()} className="w-9 h-9 flex items-center justify-center bg-red-50 text-red-600 hover:bg-red-100 rounded-lg"><LogOut size={18} /></button>
          </div>
        </div>
      </header>
          ...
          <div className="relative z-10">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
              <div className="flex items-center gap-3 sm:gap-4 flex-1">
                <div className={`p-3 sm:p-4 rounded-xl ${deliveryStatus.iconBgColor} backdrop-blur-sm`}><StatusIcon size={24} className={deliveryStatus.iconColor} /></div>
                <div className="flex-1 min-w-0"><div className="text-[10px] sm:text-xs text-indigo-200 font-bold uppercase tracking-wide">Delivery Status</div><div className="text-lg sm:text-2xl font-black mt-0.5 truncate">{deliveryStatus.title}</div><div className="text-xs sm:text-sm text-indigo-200 mt-0.5">{deliveryStatus.subtitle}</div></div>
              </div>
              <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto">
                <div className="flex-1 sm:flex-none bg-white/15 backdrop-blur-sm rounded-xl px-4 py-2.5 sm:py-3 text-center border border-white/10"><div className="text-xl sm:text-2xl font-black">{remainingToday}</div><div className="text-[10px] sm:text-xs text-indigo-200">Remaining</div></div>
                {profile.payment_status === 'active' && !isExpired && (
                  <button onClick={toggleDeliveryPause} disabled={refreshing} className={`flex-1 sm:flex-none backdrop-blur-sm px-4 py-2.5 sm:py-3 rounded-xl text-xs sm:text-sm font-bold flex items-center justify-center gap-1.5 border ${isPaused ? 'bg-green-500/30 border-green-400/30 text-green-100' : 'bg-white/15 border-white/10 text-white'}`}>{refreshing ? <div className="w-4 h-4 border-2 border-current border-t-transparent rounded-full animate-spin" /> : isPaused ? <><Play size={14} /><span>Resume</span></> : <><Pause size={14} /><span>Pause</span></>}</button>
                )}
              </div>
            </div>
            {/* Progress Bars */}
                  ...
                  <div key={lead.id} className="p-3 sm:p-4 hover:bg-slate-50/50 transition-colors">
                    <div className="flex justify-between items-start mb-2 sm:mb-3">
                      ...
                      <div className={`px-2 py-1 rounded-lg text-[10px] sm:text-xs font-bold border ml-2 flex items-center gap-1 ${isNightLead ? 'bg-indigo-50 border-indigo-100 text-indigo-700' : 'bg-slate-50 border-slate-200 text-slate-600'}`}>
                        ...
                        {!isToday(lead.created_at) && <span className="hidden sm:inline opacity-60 ml-1">(Yest)</span>}
                      </div>
                    </div>
                      ...
                      <div className="mb-3 bg-blue-50 border border-blue-100 rounded-lg p-2.5 flex gap-2.5 items-start">
                        ...
                        <div>
                          <p className="text-xs text-blue-800 font-medium leading-relaxed">
                            <span className="font-bold">Pro Tip:</span> Night Lead! Call pick hone ke chances kam ho sakte hain, par Conversion High ho sakta hai.
                          </p>
